...
'''' POV-Surgery Dataset class '''

class DatasetPOVSurgery(data.Dataset):
    """# Dataset Class """
    ...
    def __init__(self, root='./', load_set='train', transform=None, num_kps3d=21, num_verts=778, hdf5_file=None, other_params={}):
        ...
        # TODO: add depth transformation
        self.load_set = load_set  # 'train','val','test'
        with open(os.path.join(root, f'images-{self.load_set}.pkl'), 'rb') as f: 
        ...
        # self.mesh2d = np.load(os.path.join(root, 'mesh2d-%s.npy' % self.load_set), allow_pickle=True)
        # self.mesh2d = self.mesh2d.astype(np.float64) if self.mesh2d.dtype == object else self.mesh2d
        with open(os.path.join(root, f'mesh3d-{self.load_set}.pkl'), 'rb') as f:
            self.mesh3d = pickle.load(f)
        with open(os.path.join(root, f'boxes-{self.load_set}.pkl'), 'rb') as f:
            self.boxes = pickle.load(f)
        with open(os.path.join(root, f'keypoints2d-{self.load_set}.pkl'), 'rb') as f: 
            self.keypoints2d = pickle.load(f)
        with open(os.path.join(root, f'keypoints3d-{self.load_set}.pkl'), 'rb') as f:
            self.keypoints3d = pickle.load(f)
    ...
    def __getitem__(self, index):
        ...
        palm = self.keypoints3d[index][0]
        point2d = self.keypoints2d[index][:, :2] # get only first 2 columns, visibility will be added later in create_rcnn_data()
        point2d[:, [0, 1]] = point2d[:, [1, 0]] # swap keypoints2d x-y: (x, y) -> (y, x)
        point3d = self.keypoints3d[index] - palm # Center around palm
        ...
        data = {
            ...
            'original_image': original_image,
            'inputs': inputs,
            # 'point2d': point2d, # TODO: remove
            # 'point3d': point3d, # TODO: remove
            # 'mesh2d': mesh2d, # TODO: remove
            # 'bb': bb, # TODO: remove
            'boxes': boxes,
            'labels': labels,
            'keypoints': keypoints,
