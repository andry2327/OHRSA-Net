...
class AutoBackend(nn.Module):
    ...
    def forward(self, im, augment=False, visualize=False, embed=None):
        ...
        if self.pt or self.nn_module:
            y, feature_maps = self.model(im, augment=augment, visualize=visualize, embed=embed)
        ...
        if isinstance(y, (list, tuple)):
            y_ = self.from_numpy(y[0]) if len(y) == 1 else [self.from_numpy(x) for x in y]
            return (y_, feature_maps)
        else:
